      ...
      <header className="pt-10 pb-8 px-10 flex flex-col gap-8 relative z-10">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div className="space-y-1">
             ...
             <h1 className="text-4xl font-bold tracking-tight text-foreground font-outfit">Contacts Hub</h1>
          ...
          <div className="flex items-center gap-4">
             <div className="relative group">
                <Search className="w-4 h-4 absolute left-4 top-1/2 -translate-y-1/2 text-muted-foreground/70 group-focus-within:text-foreground transition-colors" />
                <input 
                  type="text"
                  placeholder="Scan directory..."
                  value={searchTerm}
                  onChange={(e) => setSearchTerm(e.target.value)}
                  className="pl-11 pr-5 py-3.5 bg-surface border border-border-muted rounded-2xl text-sm font-medium text-foreground focus:outline-none focus:ring-1 focus:ring-foreground/10 w-64 transition-all placeholder:text-muted-foreground/90"
                />
             </div>
        ...
        <div className="flex items-center gap-3 overflow-x-auto pb-2 custom-scrollbar">
           ...
           <button className="px-4 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-widest bg-surface text-muted-foreground/90 border border-border-muted hover:border-border transition-all">Recruiters</button>
           <button className="px-4 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-widest bg-surface text-muted-foreground/90 border border-border-muted hover:border-border transition-all">Stakeholders</button>
           <button className="px-4 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-widest bg-surface text-muted-foreground/90 border border-border-muted hover:border-border transition-all">Referrals</button>
        </div>
      </header>
        ...
        {isLoading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-8">
            {[1, 2, 3, 4, 5, 6, 7, 8].map(i => (
              <div key={i} className="h-64 bg-surface/50 animate-pulse rounded-[2rem] border border-border-muted" />
            ))}
          </div>
          ...
          >
             <div className="w-20 h-20 rounded-[2rem] bg-surface border border-border-muted flex items-center justify-center text-muted-foreground/90">
               <UserPlus className="w-10 h-10" />
             </div>
             <div className="space-y-1">
                <p className="text-xl font-bold text-foreground">No Contacts</p>
                <p className="text-sm text-muted-foreground max-w-xs mx-auto">No contacts found matching your criteria. Add new professional contacts to expand your reach.</p>
             </div>
             <button 
                onClick={() => setIsModalOpen(true)}
                  ...
                  <div className="flex justify-between items-start">
                    ...
                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-all duration-300">
                      <button 
                        ...
                        className="p-2 text-muted-foreground/80 hover:text-foreground hover:bg-background rounded-xl transition-all"
                      >
                        <Pencil className="w-4 h-4" />
                      </button>
                      <button 
                        onClick={() => handleDelete(contact.id)}
                        className="p-2 text-muted-foreground/90 hover:text-red-500 hover:bg-red-500/10 rounded-xl transition-all"
                      >
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {contact.category === 'POI' && (
                        <span title="Point of Influence">
                          <ShieldCheck className="w-3.5 h-3.5 text-foreground/60" />
                        </span>
                      )}
                    </div>
                    <div className="flex items-center gap-2">
                       <span className={`text-[9px] font-black px-2.5 py-1 rounded-full uppercase tracking-widest border shrink-0 transition-colors ${
                         contact.category === 'HR' ? 'bg-surface border-border-muted text-muted-foreground/90' : 'bg-foreground text-background border-foreground'
                       }`}>
                         {contact.category === 'HR' ? 'Recruiter' : 'Influencer'}
                       </span>
                       <div className="flex items-center gap-1.5 text-[11px] font-bold text-muted-foreground/80 truncate">
            ...
            >
              ...
              <div className="relative z-10 px-10 pt-10 pb-6 flex items-center justify-between">
                <div className="space-y-1">
                  ...
                  <p className="text-muted-foreground text-sm font-medium italic">Add a new professional contact.</p>
                </div>
              ...
              <form onSubmit={handleSubmit} className="relative z-10 p-10 pt-0 space-y-8">
                <div className="space-y-6">
                  <div className="space-y-2">
                    <label className="text-[11px] font-bold text-muted-foreground/90 uppercase tracking-widest ml-1">Contact Name</label>
                    <div className="relative">
                      <UserCircle2 className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground/70" />
                      <input required value={name} onChange={e => setName(e.target.value)} placeholder="Jane Doe" className="w-full bg-surface border border-border-muted rounded-2xl pl-12 pr-5 py-4 text-sm font-medium focus:outline-none focus:ring-1 focus:ring-foreground/10 transition-all placeholder:text-muted-foreground/90" />
                    </div>
                  </div>
                  
                  <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-2">
                      <label className="text-[11px] font-bold text-muted-foreground/90 uppercase tracking-widest ml-1">Category</label>
                    ...
                    <div className="space-y-2">
                      <label className="text-[11px] font-bold text-muted-foreground/90 uppercase tracking-widest ml-1">Company</label>
                      <div className="relative">
                        <Building2 className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground/70" />
                        <input value={company} onChange={e => setCompany(e.target.value)} placeholder="Stripe, Vercel..." className="w-full bg-surface border border-border-muted rounded-2xl pl-12 pr-5 py-4 text-sm font-medium focus:outline-none focus:ring-1 focus:ring-foreground/10 transition-all placeholder:text-muted-foreground/90" />
                      </div>
                    </div>
                  </div>

                  <div className="space-y-2">
                    <label className="text-[11px] font-bold text-muted-foreground/90 uppercase tracking-widest ml-1">Email Address</label>
                    <div className="relative">
                  ...
                  <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-2">
                      <label className="text-[11px] font-bold text-muted-foreground/90 uppercase tracking-widest ml-1">Phone Number</label>
                      <div className="relative">
                        <Phone className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground/70" />
                    ...
                    <div className="space-y-2">
                      <label className="text-[11px] font-bold text-muted-foreground/90 uppercase tracking-widest ml-1">LinkedIn Profile</label>
                      <div className="relative">
                ...
                <div className="pt-8 flex items-center justify-between border-t border-border-muted">
                  <button type="button" onClick={closeModal} className="text-xs font-black uppercase tracking-[0.2em] text-muted-foreground/80 hover:text-foreground transition-colors">Cancel</button>
                  <button 
                    type="submit" 
...
function ContactTrigger({ href, icon, active, label }: { href?: string, icon: React.ReactNode, active: boolean, label: string }) {
  if (!active || !href) {
    return (
      <div className="flex flex-col items-center gap-2 py-3 rounded-[1.5rem] bg-background/50 text-muted-foreground/80 border border-transparent cursor-not-allowed">
        {icon}
  ...
  return (
    <a 
      ...
      className="flex flex-col items-center gap-2 py-3 rounded-[1.5rem] bg-background border border-border-muted text-muted-foreground/80 hover:text-foreground hover:border-border transition-all group/trigger"
    >
